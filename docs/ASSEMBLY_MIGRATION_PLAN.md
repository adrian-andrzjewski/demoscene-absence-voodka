...
## Current implementation checkpoint: Phase 3B.4 (after Phase 1C and 2X)
...
Phase 3B.4 now stores the raw command-line pointer in assembly, registers the
production assembly exception-filter entry, and moves the production Escape
watcher event/thread lifecycle into NASM. Formatted crash reporting, argument
interpretation, key-map state, and the main-thread message pump remain C++.
...
libxmp for differential validation. The next gate is Phase 3B.5: assembly-side
command-line interpretation and the remaining input bridge, including scalar
and path selectors, key-map state, and the main-thread message pump. The
transition into `DemoStart32` and final subsystem orchestration remain C++ until
those gates have equivalent evidence.
